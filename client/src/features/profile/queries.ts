...
const getShow = async (id: string) => {
  return ShowQueryResponse.parse(
    (await serverClient.get(`/api/v1/show/${id}`)).data
  );
};
const getMultipleShows = async () => {
  return MultipleShowsQueryResponse.parse(
    (await serverClient.get(`/api/v1/show/`)).data
  );
};

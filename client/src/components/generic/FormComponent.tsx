...
const FormComponent = (props: FormComponentProps): JSX.Element => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    defaultValues: props.options.reduce((acc, curr) => ({
      ...acc,
      [`${curr.name}`]: curr.default ?? null,
    })),
  });
  return (
    <form
        ...
        return (
          <React.Fragment key={index}>
            ...
            {(errors as any)[`${option.name}`]?.message && (
              <div
                className={`flex items-center gap-2 text-white bg-stone-900 rounded-xl px-4 py-2 mt-2 ${props.styles?.error}`}
              >
